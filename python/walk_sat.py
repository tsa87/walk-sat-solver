...
class SAT:
    ...
    def walk_sat(self, p, max_flips):
        ...
        for i in range(max_flips):
            ...
            if random.uniform(0, 1) < p:
                ...
                def sat_count(var):
                    model[var] = not var
                    count = len([clause for clause in self.clauses if self.check_clause(clause, model)])
                    model[var] = not var
                    return count
    ...
    def check_assignment(self, model):
        ...
        for clause in self.clauses:
            (sat_clauses if self.check_clause(clause, model) else unsat_clauses).append(clause)
